...
Puts the site on the internet at readback.ca without opening a single
...
- readback.ca must be in your Cloudflare account (it is — registered
  through Cloudflare, so DNS is already there).
...
Add a second hostname the same way for `www.readback.ca` if you want the
www variant to resolve.
...
Then load **https://readback.ca** — TLS is handled by Cloudflare, nothing
to configure.
...
machine is on, awake, and online. Sleep the laptop and readback.ca goes
down.
...
`dev.readback.ca`), Pages on the apex for production.
